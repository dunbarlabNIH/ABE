...
barcode_ggheatmap <- function(your_data,
  ...
  your_data_list$prop_table = apply( your_data_list$prop_table,
                                     MARGIN = 2, 
                                     FUN = function(sample){ 
 ...
 # if (any(colSums(your_data_list$prop_table) == 0)){
  ...
  if (cellnote_option %in% c("logs", "ranks", "stars", "percents", "reads")){
    ...
    if(!is.null(libraries)){
      barcode.colors <- data.frame( matrix(ncol = 2, nrow = 0) )
      colnames(barcode.colors) <- c("barcode", "color")
      nBarcodes <- length(plotting_data$BARCODE)/ncol(your_data)
      for(library in libraries){
        libBar <- getLibraryBarcodeVector(barcodes = plotting_data$BARCODE[1:nBarcodes], library = library)
        curBarCol <- data.frame(barcode = libBar, color = rep(library$color,times = length(libBar)))
        barcode.colors = rbind(barcode.colors,curBarCol)
      }
      #reorder to be same as plotting data
      barcode.colors <- barcode.colors[match(barcode.colors$barcode, plotting_data$BARCODE[1:nBarcodes]),]
      #repeat ncol times
      barcode.colors <- rep(barcode.colors, times = ncol(your_data))
      #add to plotting data
      plotting_data$LIB_COLOR = barcode.colors$color
    }
  } else {
  ...
  if(printtable == TRUE){
    ...
    g1_heatmap <- ggplot2::ggplot(plotting_data, ggplot2::aes(x = SAMPLE, y = BARCODE))+
      ...
      #geom_bar(mapping = aes(x = 1, fill = sample.color))
